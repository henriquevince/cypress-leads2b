describe('Plataforma Leads2B: teste prospectando e acessando página de negócio da empresa', () => {
    it('Teste 3: prospectando a empresa e acessando a página de negócio (verifica também o filtro de visualziação por mapa). Deve acessar, logar na plataforma Leads, acessar a seção de consulta de CNPJ Pro, buscar empresa por filtro Mapa, prospectar e acessar página de negócio', () => {
      ...
      cy.contains('li[class="pl2 pt2 pb2"]' , 'Academias').click() //Seleciona segmento "Academias"
  
      cy.get('.a-btn--primary').click() //Clica botão "Buscar"

      cy.get('button[class="a-btn a-btn--"]').click() //Clica no filtro por mapa
    
      cy.wait(4000) //Aguarda 4s após a requisição de busca

      cy.get('input[name="checkboxFilterPurchased"]').click()
    
      cy.get('.scroll-active__cards > :nth-child(1)').click() //Clica no item da lista especificado em nth-child()
      ...
      cy.get('button[class="a-btn mb-2 mr-3 w-100 py-2 a-btn--success-secondary"]').click() //Acessa página de negócio da empresa prospectada
